...
public interface CategoryReportMapper extends Mapper<CategoryReport>{
    ...
    @Select("SELECT category_id1 categoryId1,category_id2 categoryId2,category_id3 categoryId3, " +
            "       DATE_FORMAT(o.`pay_time`,'%Y-%m-%d') countDate,SUM(oi.num) num,SUM(oi.pay_money) money " +
            "FROM tb_order_item oi, tb_order o WHERE oi.`order_id`=o.`id` AND o.`pay_status`='1' " +
...
public List<CategoryReport> categoryReport(@Param("date") LocalDate date);
   ...
   List<Map> categoryReport1(@Param("date1") String date1, @Param("date2") String date2);
